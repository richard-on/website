...
func Init() error {
	...
	if err != nil {
		return fmt.Errorf("GODOTENV init: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("LOG_CW init: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("LOG_LEVEL init: %v", err)
	}

	FiberPrefork, err = strconv.ParseBool(os.Getenv("FIBER_PREFORK"))
	if err != nil {
		return fmt.Errorf("FIBER_PREFORK init: %v", err)
	}

	MaxCPU, err = strconv.Atoi(os.Getenv("MAX_CPU"))
	if err != nil {
		return fmt.Errorf("MAX_CPU init: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("SENTRY_TSR init: %v", err)
	}
